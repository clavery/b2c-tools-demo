...
const {
    ...
    ensureDataAPIPermissions, executeJob
} = require('./jobs');
...
const B2C_TOOLKIT_DATA_VERSION = 5;
 ...
 * @property {string[]} b2cToolsBootstrappedClientIDs list of client IDs that have been bootstrapped
 */
...
async function getInstanceState(env) {
    try {
        ...
        return {
            ...
            b2cToolsBootstrappedClientIDs: resp.data.c_b2cToolsBootstrappedClientIDs ? resp.data.c_b2cToolsBootstrappedClientIDs.split(',') : []
        };
    } catch (e) {
...
`;
    ...
    try {
        await siteArchiveImport(env, zipBuffer, {
            archiveName
        });
    } catch (e) {
        if (e.response && e.response.status === 403) {
    ...
    if (!instanceState.b2cToolsBootstrappedClientIDs.includes(env.clientID)) {
        ...
        await env.ocapi.patch(`/global_preferences/preference_groups/b2cToolkit/development`, {
            c_b2cToolsBootstrappedClientIDs: instanceState.b2cToolsBootstrappedClientIDs.concat(env.clientID).join(',')
        });
    }
}
 ...
 * @return {Promise<void>}
 */
async function migrateInstance(env, dir, exclude = [], apply = true, dryRun = false, forceBootstrap = false, allowBootstrap = true) {
    ...
    var bootstrapRequired = !instanceState
        || !instanceState.b2cToolkitDataVersion
        || instanceState.b2cToolkitDataVersion < B2C_TOOLKIT_DATA_VERSION
        || !instanceState.b2cToolsBootstrappedClientIDs
        || !instanceState.b2cToolsBootstrappedClientIDs.includes(env.clientID);
    ...
    if (typeof lifeCycleModule.beforeAll === 'function') {
        ...
        await lifeCycleModule.beforeAll({
            ...
            logger,
            helpers: B2C_MIGRATION_HELPERS
        }, migrationsToApply, apply, dryRun);
    }
